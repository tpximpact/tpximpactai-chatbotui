...
# Azure Enviornment Variables - Chunk & Summarise Sizes
...
print("cosmosdb key", AZURE_COSMOSDB_ACCOUNT_KEY)
frontend_settings = {
...
def init_container_client(storage_container_name):
    ...
    blob_service_client = BlobServiceClient(
        account_url=storage_account_url, credential=AZURE_STORAGE_KEY
    )
...
def init_vector_store():
    ...
    try:
        ...
        fields = [
            SimpleField(
                name="id", type=SearchFieldDataType.String, key=True, sortable=True
            ),
            SearchableField(
        ...
        scoring_profiles = [
            {
                ...
                "text": {"weights": {"content": 100, "content_vector": 100}},
                "functions": [],
            }
        ]
...
def get_configured_data_source(user_id, filenames):
    ...
    if DATASOURCE_TYPE == "AzureCognitiveSearch":
        ...
        logging.debug(
            "Using system-assigned managed identity for Azure Cognitive Search"
        )
...
def prepare_model_args(request_body):
    ...
    for index, message in enumerate(reversed(request_messages)):
        if message and message["role"] != "tool":
            ...
            if totalTokens > 6000:
                print(
                    "Message list too long, truncating to 6000 tokens which is ",
                    index,
                    " messages.",
                )
                break
            messages.insert(
                0,
                {"role": message["role"], "content": cleanMessage(message["content"])},
            )
    ...
    if len(request_filenames) == 0:
        print("no files")
        messages.insert(0, {"role": "system", "content": AZURE_OPENAI_SYSTEM_MESSAGE})
    elif len(request_filenames) > 0:
        print("Request filenames: ", request_filenames)
        editted_messages = messages.copy()
        editted_messages[-1]["content"] = (
            "You are answering questions about the following documents: "
            + ", ".join(request_filenames)
            + ". "
            + editted_messages[-1]["content"]
        )
...
def get_doc_from_azure_blob_storage(
    blob_name: str, storage_account_container: str
) -> List[Document]:
    ...
    try:
        ...
        file_path = os.path.join(
            temp_dir, f"{storage_account_container[:10]}-{rand_int}-{safe_blob_name}"
        )
